...
<div class="container">
    ...
    @php // Use URL student_id or fallback to logged-in user's student_id
        ...
        $mentorNames = $locations 
            ? collect([$locations->mentor_id1, $locations->mentor_id2])
                ->filter()
        ...
        <form action="{{ url('student/log/' . $student->student_id) }}" method="POST" id="logForm">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    ...
                    @if ($locations)
                        <ul class="list-group">
                        @foreach(collect([$locations->mentor_id1, $locations->mentor_id2])->filter() as $mentorId)
                            @php
    ...
    @if(auth()->user()->role === 'Student')
        ...
        @else
            <div class="alert alert-warning text-center" role="alert">
                ท่านยังไม่ได้ลงทะเบียนสถานที่ฝึกงาน กรุณาแจ้งอาจารย์ที่รับผิดชอบ
            </div>
        @endif
    @endif
    ...
    @if(!$locations && auth()->user()->role === 'Student')
    @else
